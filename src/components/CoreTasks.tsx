...
function getCardClass(dimmed: boolean, done: boolean, neverSkip: boolean): string {
  if (dimmed && !neverSkip) {
    return 'opacity-40 cursor-not-allowed bg-gray-50 dark:bg-gray-800/50 border-gray-100 dark:border-gray-800'
  }
  if (neverSkip && !done) {
    return 'bg-red-50 dark:bg-red-900/10 border-red-200 dark:border-red-900 cursor-pointer hover:border-red-300 dark:hover:border-red-700'
  }
  if (done) {
...
function getCheckboxClass(done: boolean, neverSkip: boolean): string {
  if (done) return 'bg-green-500 border-green-500'
  if (neverSkip) return 'border-red-400 dark:border-red-600'
...
function isTaskDone(dayKey: string, taskId: string): boolean {
  try {
    return (
      localStorage.getItem(`habits:${dayKey}:${taskId}:${getTodayISO()}`) === 'true'
    )
  } catch {
    return false
  }
...
function CoreTaskCard({
  ...
  const isActuallyDimmed = dimmed && !neverSkip

  return (
<button
  type="button"
  onClick={isActuallyDimmed ? undefined : toggle}
  disabled={isActuallyDimmed}
  className={`flex w-full items-center gap-3 rounded-xl px-4 py-3 border transition-all duration-200 text-left ${getCardClass(dimmed, done, neverSkip)}`}
>
      {/* Icon */}
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2 mb-0.5">
          ...
          {neverSkip && (
            <span className="text-xs font-semibold px-1.5 py-0.5 rounded-full bg-red-100 text-red-600 dark:bg-red-900/40 dark:text-red-400 shrink-0">
              always
            </span>
          )}
        </div>
      ...
      {/* Checkbox */}
      <div className={`w-5 h-5 rounded-md border-2 flex items-center justify-center shrink-0 transition-all duration-200 ${getCheckboxClass(done, neverSkip)}`}>
        {done && (
...
export function CoreTasks({ dayKey, mode }: Props) {
  ...
  const neverSkipDone = coreTasks
    .filter((t) => t.neverSkip)
    .every((t) => isTaskDone(dayKey, t.id))
