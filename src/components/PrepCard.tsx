...
const typeStyle: Record<string, { icon: string; card: string; badge: string; iconBg: string }> = {
  ielts: {
    icon: 'I',
...
export function PrepCard({ session, dayKey }: Props) {
  ...
  return (
    <button
      onClick={toggle}
      type="button"
      className={`rounded-xl border px-4 py-3 flex flex-col gap-3 cursor-pointer transition-all duration-200 text-left ${
        done
    ...
    >
      ...
      <div className="flex items-center gap-3">
        <div
          className={`w-8 h-8 rounded-lg flex items-center justify-center text-sm font-bold shrink-0 ${style.iconBg}`}
        >
          {session.icon}
        </div>
      ...
      {!done && (
        <ul className="flex flex-col gap-1.5 pl-1">
          {session.tasks.map((task) => (
            <li
              key={task.id}
              className="flex items-baseline gap-2 text-xs text-gray-500 dark:text-gray-400"
            >
